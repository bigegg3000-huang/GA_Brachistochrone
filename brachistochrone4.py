...
# 3.1. 출력(Output): 출력
    ...
    ax2.set_title('Fitnesses of Population')
    ax2.legend()
    ...
    def animate(generation):
        ...
        line1.set_data(x, y)
        text1.set_text(f'Generation: {generation}\nFitness: {gen_maxfitness[generation]:.2f}')

        line2_max.set_data(generation, gen_maxfitness[generation])
        line2_ave.set_data(generation, gen_avefitness[generation])
        line2_min.set_data(generation, gen_minfitness[generation])
    ...
    # 애니메이션 생성
    ani = FuncAnimation(fig, animate, frames=range(max_gen), init_func=init, blit=True, interval=10)

    # 그래프 제목 설정
    fig.suptitle('Brachistochrone Curve Evolution and Fitnesses of Population')  # 원하는 제목으로 변경 가능
    ...
    ax.axhline(0, color='black', linewidth=0.5)  # x 축
    ax.axvline(0, color='black', linewidth=0.5)  # y 축
    ax.set_xlim(0, r*np.pi)
    ax.set_ylim(0, 2*r)
    line, = ax.plot([], [])
    text = ax.text(0.5, 0.05, '', transform=ax.transAxes, fontsize=10, ha='center')
    ...
    def animate(generation):
        ...
        line.set_data(x, y)
        text.set_text(f'Generation: {generation}\nFitness: {gen_maxfitness[generation]:.2f}')
        return line, text
    
    ani = FuncAnimation(fig, animate, frames=range(max_gen), init_func=init, blit=True, interval=10)
    plt.title('Brachistochrone Curve Evolution')
    plt.show()
    """
